...
#include "kawpow/include/ethash.h"
#include "kawpow/include/ethash.hpp"
#include "kawpow/include/progpow.hpp"
#include "kawpow/uint256.h"
#include "kawpow/helpers.hpp"
...
DECLARE_FUNC(kawpow) {
    if (info.Length() < 5)
        RETURN_EXCEPT("KawPOW Hash - 5 arguments expected.");
   ...
   SET_BUFFER_RETURN(to_hex(*hash_out_ptr).c_str(), 32);
}

DECLARE_FUNC(kawpow_verify) {
     if (info.Length() < 5)
         RETURN_EXCEPT ("KawPOW Verify - 5 arguments expected.");
...
NAN_MODULE_INIT(init) {
    ...
    NAN_EXPORT(target, kawpow_verify);
